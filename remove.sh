#!/bin/bash
rm -rf ~/.config/neofetch ~/.config/rofi
mkdir ~/.config/neofetch
mkdir ~/.config/rofi
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-keyboard-shortcuts.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-power-manager.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-screensaver.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-terminal.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml
rm -rf ~/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml
rm ~/.bashrc
rm /etc/default/grub